...
var apiUrls = {
  production: 'https://salty-brushlands-12374.herokuapp.com',
...
var onNewQuestion = function onNewQuestion(event) {
    if (event) {
        event.preventDefault();
    }
    api.getQuizzes().then(ui.newQuestion).catch(ui.apiFailure);
};

var onAnswerQuestion = function onAnswerQuestion(event) {
    event.preventDefault();
    var consecutiveCorrect = getConsecutiveCorrect(event);

    if (consecutiveCorrect >= 3) {
        // delete quiz and congratulate user for learning capital
        api.deleteQuiz().then(ui.deleteSuccess).catch(ui.apiFailure);
    } else {
        ...
        .then(ui.updateQuizSuccess).catch(ui.apiFailure);
    }
};

// determine how many times the user has correctly answered the quiz question
var getConsecutiveCorrect = function getConsecutiveCorrect(event) {
    var data = getFormFields(event.target);
    // determine whether answer is correct based on the stored value of quiz answer
    var isCorrect = data.answer == store.quiz.card.side_b;
    var consecutiveCorrect = store.quiz.consecutive_correct;
    isCorrect ? consecutiveCorrect++ : consecutiveCorrect = 0;
    return consecutiveCorrect;
};

var updateApiJSON = function updateApiJSON(consecutiveCorrect) {
    var apiData = {
        quiz: {
...
var quizzesEvents = __webpack_require__(4);
...
var addHandler = function addHandler() {
  ...
  $('#sign-up-button').on('click', function () {
    return $('#sign-up-modal').css('display', 'block');
  });
  $('#sign-in-button').on('click', function () {
    return $('#sign-in-modal').css('display', 'block');
  });
  $('#change-password-button').on('click', function () {
    return $('#change-password-modal').css('display', 'block');
  });
...
var store = __webpack_require__(1);
var quizEvents = __webpack_require__(4);

var mismatchedPasswords = function mismatchedPasswords() {
...
var signUpFailure = function signUpFailure(err) {
  document.getElementById('sign-up').reset();
...
var signInSuccess = function signInSuccess(data) {
  ...
  setTimeout(function () {
    return $('#flashcard').css('display', 'block');
  }, 1100);
...
var changePasswordFailure = function changePasswordFailure(err) {
  document.getElementById('change-password').reset();
...
var getQuizzes = function getQuizzes() {
    return $.ajax({
...
var newQuestion = function newQuestion(data) {
    ...
    var selector = Math.floor(data.quizzes.length * Math.random());
    var quiz = data.quizzes[selector];
    store.quiz = quiz;
    $('#question').text('' + quiz.card.side_a);
};

var updateQuizSuccess = function updateQuizSuccess(data) {
...
var apiFailure = function apiFailure(error) {
